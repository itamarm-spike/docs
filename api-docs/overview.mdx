...
Spike API provides a seamless way to integrate user data from various providers like Garmin and Fitbit into your application. Here's a quick guide to understanding the flow of using the Spike RESTful API:
...
4. **Data Retrieval Using Queries**
   ...
   Spike API offers multiple endpoints for data retrieval, allowing you to access a wide range of user data. For detailed information on querying data, refer to the following documentation:

   - [Statistics API documentation](/api-docs/statistics)
   - [Sleep API documentation](/api-docs/sleep)
   - [Workout API documentation](/api-docs/workout)
   - [Provider Records API documentation](/api-docs/provider_records)

```mermaid
graph TD;
    A[Start] --> B[Generate HMAC Signature];
    B --> C[Exchange Signature for Access Token];
    C --> D[Obtain Redirect URL];
    D --> E[Redirect the User];
    E --> F[Configure Webhook Endpoint];
    F --> G[Handle Webhook Events];
    G --> H[Data Retrieval Using Queries];
    H --> I[End];
```
